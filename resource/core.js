...
Core.config = {
    ...
    serverUrl: 'http://192.168.1.177:89'
};
//var data = [[{}, {}, {}], [{}]];
//桌面右键菜单
Core.menu={
...
Core.parseQueryString = function (url) {
	var reg_url = /^[^\?]+\?([\w\W]+)$/,
...
Core.init = function(){
    ...
    // $("#desk ul li").on('mouseenter', function () {
    //     $(this).find('.bg-item').css({'display' : 'block'});
    //     if ($(this).find('.name-application')[0].clientHeight < 30) {
    //         $(this).find('.bg-item').css({'height' : '133px'});
    //     }
    //     // $(this).find('.name-application').css({'background-color' : '#0080a9'})
    // });
    // $("#desk ul li").on('mouseleave', function () {
    //     // if ($(this).find('.name-application')[0].clientHeight > 30) {
    //     //
    //     // }
    //     $(this).find('.bg-item').css({'display' : 'none'});
    //     // $(this).find('.name-application').css({'background-color' : ''})
    // });
    /**/
    $(window).bind('load',function(){
        //循环输出每个图标
        var qs = Core.parseQueryString(window.location.href);
        // $.ajax({
        //     url: Core.config.serverUrl + '/Ajax/ApplyHandler.ashx',
        //     type: 'GET',
        //     data: {
			// 	MethedName: 'getuserapply',
			// 	userid: qs.userId || '304edebc-b49d-4162-b6d2-889defc8ab5c'
        //     },
        //     success: function(res){
        //         var data = JSON.parse(res);
        //         if(data.code === '200') {
        //             var shortcutList = shortcut = data.data;
			// 		for(var sc in shortcutList){
			// 			_cache.shortcutTemp = {
			// 				"top":_top,
			// 				"left":_left,
			// 				"title":shortcutList[sc]['APPLY_NAME'],
			// 				"shortcut":shortcutList[sc]['APPLY_ID'],
			// 				"imgsrc": shortcutList[sc]['APPLY_IMAGE'],
			// 				"width": shortcutList[sc]['APPLY_WIDTH'],
        //                     "height": shortcutList[sc]['APPLY_HEIGHT']
			// 			};
...
Core.create = function(obj,opt){    
    ...
    if(typeof(obj)==='string'){
        ...
        var options = {
            num		:obj['SORTCODE'],
            title	:obj['APPLY_NAME'],
            imgsrc	:obj['APPLY_IMAGE'],
            url		:obj['APPLY_URL'],
            width	: +obj['APPLY_WIDTH'] || 800,
            height: +obj['APPLY_HEIGHT'] || 500,
            resize	:true
        };
    }
    if (obj['APPLY_POSITION'] === '2') {
        window.open(obj['APPLY_URL']);
        return;
    }
